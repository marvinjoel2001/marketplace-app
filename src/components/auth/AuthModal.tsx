      ...
      <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {authModalStep === 'login' && subView === 'main' && (
          <div className="p-6 sm:p-8 space-y-5">
            ...
            {/* SELECTOR PRINCIPAL: SOY COMPRADOR vs SOY TIENDA */}
            <div className="grid grid-cols-2 gap-1.5 p-1 bg-slate-100 rounded-2xl text-xs font-black">
              ...
              >
                ...
                <span>Soy Tienda / Admin</span>
              </button>
            </div>

            {/* SI ES TIENDA: ACCESO DIRECTO AL PANEL DE ADMINISTRACIÓN */}
            {loginRole === 'vendor' && (
              <div className="space-y-3 pt-1 animate-in fade-in">
                <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-100 text-xs text-slate-700">
                  <span className="font-black text-emerald-900 block mb-0.5">Acceso a Panel de Tienda</span>
                  <span>Ingresa para gestionar tu catálogo, inventario, arqueo de caja POS y activar ventas en TikTok Live.</span>
                </div>

                <div className="space-y-2">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">
                    Tiendas oficiales registradas en Santa Cruz:
                  </span>
                  {[
                    { id: 's-1', name: 'TechPlus Bolivia', slug: 'techplus-bolivia', tiktok: '@techplus_bo', city: 'Equipetrol' },
                    { id: 's-2', name: 'ModaBol Santa Cruz', slug: 'modabol-santacruz', tiktok: '@modabol_oficial', city: 'Centro' },
                    { id: 's-4', name: 'AudioPro Santa Cruz', slug: 'audiopro-santacruz', tiktok: '@audiopro_scz', city: 'Torre Dúo' },
                  ].map((st) => (
                    <button
                      key={st.id}
                      type="button"
                      onClick={() => {
                        setActiveStore({ id: st.id, name: st.name, slug: st.slug });
                        closeAuthModal();
                        router.push('/vendor');
                      }}
                      className="w-full p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-emerald-500 rounded-2xl text-left transition-all flex items-center justify-between group shadow-2xs"
                    >
                ...
                <div className="pt-2">
                  <Link
                    ...
                    onClick={() => closeAuthModal()}
                    className="w-full py-3 bg-slate-900 hover:bg-black text-white font-bold text-xs rounded-full transition-all flex items-center justify-center space-x-2 shadow-xs"
                  >
                    <Store className="w-4 h-4 text-emerald-400" />
                    <span>Registrar Nueva Tienda en Vitrina</span>
                  </Link>
                </div>
              </div>
            )}

            {/* SI ES COMPRADOR: MUESTRA LAS OPCIONES SOCIAL / DIRECTO */}
            {loginRole === 'customer' && (
              <>
                  ...
                  <form onSubmit={handleDirectSubmit} className="space-y-3 pt-1 text-xs">
                    ...
                    >
                      {isLoggingInDirect ? (
                        ...
                        <>
                          <span>{language === 'es' ? 'Ingresar como Comprador' : 'Sign in as Buyer'}</span>
